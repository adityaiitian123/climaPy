...
def render_map_view(ds, controls):
    ...
    st.markdown("""
    <div style='margin-top:2rem; padding:1rem; background:rgba(15,23,42,0.4); border:1px solid rgba(56,189,248,0.2); border-radius:12px;'>
        <h4 style='margin:0; color:#38bdf8; font-size:1.1rem;'>🔬 Scientific Static Analysis</h4>
        <p style='color:#94a3b8; font-size:0.8rem;'>High-fidelity Matplotlib visualization for scientific documentation.</p>
    </div>
    """, unsafe_allow_html=True)
    
    sci_mode = st.radio("Intelligence Mode", ["Real-World Observation", "Synthetic Simulation"], horizontal=True, key="sci_mode_main")
    ...
    if sci_mode == "Synthetic Simulation":
        ...
        Z = (2.5 * np.exp(-((LON - 78)**2 / 40 + (LAT - 23)**2 / 25)) +
        ...
        cbar.set_label("Intensity Value", color="#94a3b8")
        cbar.ax.yaxis.set_tick_params(color="#94a3b8")
        plt.setp(plt.getp(cbar.ax.axes, 'yticklabels'), color='#94a3b8')
        ax.set_title("Synthetic Spatial Intensity Simulation", color="#e2e8f0", size=10)
        ...
        if ds is not None:
            ...
            ax.set_title(f"Scientific Observation: {var}", color="#e2e8f0", size=10)
            ax.tick_params(colors='#64748b', labelsize=8)
            st.pyplot(fig)
            plt.close(fig)
